...
ForgePlugins.OrientationKeeper.prototype =
  {
    ...
    boot: function () {
      ...
      this._camera.onOrientationChange.add(this.cameraChangeHandler, this);
    ...
    cameraChangeHandler: function () {
      console.log("Camera yaw is", this._camera.yaw);
      this._camera.yaw = this._camera.yaw + FORGE.Math.degToRad(this._sceneAdjustmentYaw);
      console.log("Camera yaw is now", this._camera.yaw);
    },

    update: function () {
      //console.log(this._camera.yaw);
      //this._camera.yaw = 20;
      if (FORGE.Device.gyroscope === false) {
        return;
      }
      // var display = this.viewer.renderer.display;
    ...
    gyroHandler: function (event) {
      ...
      if (this._sceneAdjustmentQuat !== null) {
        //this._posQuatFinal.multiply(this._sceneAdjustmentQuat);
        //this._posQuatFinal.multiply(new THREE.Quaternion(-this._sceneAdjustmentQuat.y, -this._sceneAdjustmentQuat.x, -this._sceneAdjustmentQuat.z, this._sceneAdjustmentQuat.w));
      }
    },
    ...
    sceneLoadHandler: function () {
      ...
      window.yawOffset = 0;
      this._sceneAdjustmentQuat = null;
      if (!this.viewer.story.scene.config.startingYaw) {
      ...
      setTimeout(this.changeSceneOrientation.bind(this), 2500);
      //quaternion = new THREE.Quaternion().setFromAxisAngle(this._yAxis, angleOfRotation );
      //setTimeout(() => this._camera.lookAt(45),2);
    },
    ...
    changeSceneOrientation: function() {
      ...
      this._sceneAdjustmentQuat.setFromAxisAngle( new THREE.Vector3( 0, 1, 0 ), FORGE.Math.degToRad(this._currentTargetYaw-currentYaw+360));
